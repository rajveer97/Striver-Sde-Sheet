...
int heightOfTheTree(vector<int>& inorder, vector<int>& levelOrder, int N){
    ...
    // Inordered Node value -> index in inorder array
    ...
    {   // root is the last element in inorder traversal 
        // then it has no right subtree
        if(mp[levelOrder[0]] == inorder.size()-1)
            q.push({0, (int)inorder.size()-2, 1});
        
        //root is the first element in inorder traversal 
    ...
    while(!q.empty())
    {
        ...
        {   // if it is the leaf node i.e. no left subtree and no right subtree 
            ...
            // root is the last element in inorder traversal then 
            // it has no right subtree
            else if(mp[levelOrder[k]] == temp[1])
                q.push({temp[0], temp[1]-1, temp[2]+1});
            
            // root is the first element in inorder traversal then 
